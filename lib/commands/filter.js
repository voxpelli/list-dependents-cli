...
async function setupCommand (name, description, args, { pkg }) {
  const options = /** @satisfies {import('peowly').AnyFlags} */ ({
    ...
    ...sortFlags,
    exclude: {
      description: 'The name of a module to exclude (repeatable)',
      listGroup: 'Filter options',
      type: 'string',
      multiple: true,
    },
    include: {
      description: 'The name of a module to not filter out',
      listGroup: 'Filter options',
      type: 'string',
      multiple: true,
    },
  ...
  await pipeline(
    ...
    async function * (collection) {
      for await (const item of collection) {
        ...
        if (!('name' in item) || typeof item.name !== 'string' || exclude.includes(item.name)) {
          continue;
        }
    ...
    if (result.length > maxCount) {
      ...
      if (include) {
        ...
        if (included.size < include.length) {
          ...
          const additional = result.slice(maxCount).filter(({ name }) => remaining.has(name));
