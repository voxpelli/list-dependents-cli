...
import { ndjsonOutputWithAttribution } from '../utils/ndjson-output-with-attribution.js';
...
async function processData (spinner, tmpFile, context) {
  ...
  await pipeline(
    ...
    async collection => {
      if (streaming) {
        await ndjsonOutputWithAttribution(collection, modifyInPlace ? tmpFile : output, {
  ...
  if (result) {
    ...
    if (markdown || prettyPrint) {
      ...
      await ndjsonOutputWithAttribution(finalResult, output, { sourceFile });
    }
