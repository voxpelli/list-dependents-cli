...
async function updateData (spinner, itemsByName, context) {
  ...
  const partialFile = output ? path.resolve(cwd(), output + '.partial') : undefined;
  ...
  try {
    for await (const item of generator) {
      ...
      if (tempFile) {
        ...
        await appendFile(tempFile, JSON.stringify(item) + '\n', 'utf8');
      }
    ...
    if (!check) {
      if (tempFile && output) {
        ...
        await rename(tempFile, path.resolve(cwd(), output));
