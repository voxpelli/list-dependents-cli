...
async function updateData (spinner, itemsByName, context) {
  ...
  // Use a temporary file to write incrementally, protecting against data loss on error
  const tempFile = output && !check ? temporaryFile({ extension: 'ndjson' }) : undefined;
  const outputPath = output ? path.resolve(cwd(), output) : undefined;
  const partialFile = tempFile && outputPath ? outputPath + '.partial' : undefined;

  // Create a write stream for efficient incremental writing
  // eslint-disable-next-line security/detect-non-literal-fs-filename -- tempFile is from temporaryFile()
  const writeStream = tempFile ? createWriteStream(tempFile, { encoding: 'utf8' }) : undefined;
  ...
  try {
    for await (const item of generator) {
      ...
      // Write incrementally to temp file
      if (writeStream) {
    ...
    // Close write stream before moving file
    if (writeStream) {
      await new Promise((resolve, reject) => {
        writeStream.end((/** @type {Error | undefined} */ err) => {
          if (err) {
            reject(err);
          } else {
            // @ts-ignore - resolve can be called without arguments
            resolve();
          }
        });
      });
    ...
    if (!check) {
      if (tempFile && outputPath) {
        ...
        // Clean up any partial file from previous failed run
        if (partialFile) {
          try {
            // eslint-disable-next-line security/detect-non-literal-fs-filename
            await unlink(partialFile);
          } catch {
            // Ignore if partial file doesn't exist
          }
        }
      } else {
        // Fallback to original behavior for stdout
    ...
    // Close write stream before handling error
    if (writeStream) {
      writeStream.end();
    }

    // On error, save temp file as partial for resume capability
    if (tempFile && partialFile) {
      try {
        // eslint-disable-next-line security/detect-non-literal-fs-filename
        await rename(tempFile, partialFile);
        // eslint-disable-next-line no-console
        console.error(`\nPartial data saved to: ${partialFile}`);
        // eslint-disable-next-line no-console
        console.error(`To resume, run: list-dependents list -i ${partialFile} -o ${output} ${moduleName}`);
      } catch {
        // If we can't save partial file, clean up temp file
        try {
          // eslint-disable-next-line security/detect-non-literal-fs-filename
