...
async function doTheWork (context) {
  ...
  // TODO: Improve ora progress tracking. Eg. increase count as dependents are resolved?
  const spinner = ora({
    prefixText: `Listing dependents for ${moduleName}`,
    isSilent,
  }).start();
  ...
  spinner.succeed(`${result.length} dependents listed`);
}
