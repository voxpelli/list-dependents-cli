...
export const outputFlags = /** @satisfies {Record<string, import("peowly").AnyFlag & { listGroup: 'File options' }>} */ ({
  ...
  'no-source-file': {
    description: 'Do not write a sibling ATTRIBUTION.md file when writing NDJSON output to a file',
    listGroup: 'File options',
    type: 'boolean',
    'default': false,
  },
...
export function validateFileFlags (flags, targetFile, moduleName) {
  ...
  if (!('input' in flags)) {
    return { output, sourceFile: !noSourceFile };
  }

  return {
    ...validateInputFlags(flags, targetFile, { namedOutput, filePath: output }),
